...
import time
import datetime
import Conv2D_preprocessing
#from Conv2D_model import CharCNN
from Keras_Conv import  CharCNN
...
# Load data
print("Loading data...")
x, y = Conv2D_preprocessing.load_data()
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        #saver.restore(sess, tf.train.latest_checkpoint('C:/1521633883/checkpoints') )
        ...
        def dev_step(x_batch, y_batch, writer=None):
            ...
            for i in range(int(num_batches)):
                x_batch_dev, y_batch_dev = Conv2D_preprocessing.get_batched_one_hot(
        ...
        batches = Conv2D_preprocessing.batch_iter(x_train, y_train, FLAGS.batch_size, FLAGS.num_epochs)
        # Training loop. For each batch...
        for batch in batches:
            x_batch, y_batch = zip(*batch)
            print("Batch size:", np.shape(x_batch))
